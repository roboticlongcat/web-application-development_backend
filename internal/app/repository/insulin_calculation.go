...
// GET иконки корзины
func (r *Repository) GetInsulinCalculationInfo(creatorID uuid.UUID) (uint, int64, error) {
	insulinCalculationID, err := r.GetActiveInsulinCalculationID(creatorID)
	if err != nil {
	...
	count := r.GetInsulinCalculationCount(creatorID)

	return insulinCalculationID, count, nil
}

// GET список (кроме удаленных и черновика, поля модератора и создателя через логины)
func (r *Repository) GetInsulinCalculations(filters map[string]interface{}, user ds.User) ([]ds.InsulinCalculation, error) {
	...
	if !user.IsModerator {
		query = query.Where("creator_id = ?", user.User_ID)
	}
...
func (r *Repository) FormInsulinCalculation(id uint, creatorID uuid.UUID) error {
	...
	// Проверяем, что текущий пользователь - создатель расчета
	if insulinCalculation.CreatorID != creatorID {
		return errors.New("только создатель может формировать расчет")
	}
...
func (r *Repository) CompleteInsulinCalculation(id uint, status string, moderatorID uuid.UUID) error {
	...
	if err != nil {
		return errors.New("не получилось достать информацию о пользователе")
	}
...
func (r *Repository) DeleteInsulinCalculation(insulinCalculationID uint, creatorID uuid.UUID) error {
	//currentUser := auth.GetCurrentUser()
	var insulinCalculation ds.InsulinCalculation
...
func (r *Repository) GetInsulinCalculationCount(creatorID uuid.UUID) int64 {
...
func (r *Repository) GetActiveInsulinCalculationID(creatorID uuid.UUID) (uint, error) {
